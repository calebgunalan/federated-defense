...
Insider threats cost organizations more than any other category of security incident. The 2024 Ponemon Institute report puts the average annual figure at \$15.4~million, up 47\% in four years~\cite{ponemon2024}. What makes these attacks so damaging is that insiders already hold valid credentials and know the systems they are targeting---perimeter defenses are irrelevant~\cite{homoliak2019}. The range of malicious insider activity (IP theft, fraud, sabotage, unauthorized data transfers) defies any single detection signature~\cite{cappelli2012}. High-profile breaches at financial institutions and healthcare providers have only sharpened the demand for automated, cross-boundary detection systems~\cite{salem2008}. Verizon's 2023 DBIR reinforced the point: privilege misuse accounts for a growing share of confirmed breaches, and median detection time for insider incidents still exceeds 200 days~\cite{verizon2023}.

Machine learning has gained ground in this space over the past decade, driven by richer user behavior analytics (UBA) data and deeper architectures that model temporal behavioral patterns~\cite{yuan2021,liu2019,le2020}. Both supervised and unsupervised methods show promise when trained on login patterns, file access logs, email metadata, and device usage~\cite{chattopadhyay2018,singh2023}. Random Forests, SVMs, and isolation forests set early baselines, reaching 85--93\% detection on the CERT benchmark~\cite{breiman2001,gavai2015}. LSTMs and autoencoders then pushed accuracy further by exploiting sequential dependencies in user activity~\cite{tuor2017}. Yet all these methods share a bottleneck: they need large volumes of behavioral data to train well, and that data is sensitive---governed by GDPR, CCPA, labor law, and corporate policy~\cite{voigt2017}. No single organization wants to hand over its employees' activity logs to a third party, and aggregating logs across organizations raises both legal and competitive concerns~\cite{alsowail2022}.

Deep learning now dominates user behavioral analysis for insider threat scenarios. Early work after 2020 relied on transfer learning with pre-trained LSTM and GRU networks; more recent studies have moved toward specialized architectures. Liu et al.~\cite{liu2023} built a dynamic framework with attention mechanisms that weight recent behavioral changes, hitting 94.1\% AUC-ROC on CERT r6.2. Others have explored attention-enhanced networks, hybrid CNN--RNN classifiers, and multi-view fusion to combine different behavioral data streams~\cite{sanzgiri2016}. 1D convolutions turn out to be effective for short-duration anomalies---the sudden file-download spikes or after-hours login bursts that often precede exfiltration---because their local receptive fields capture patterns that recurrent models may smooth over in longer sequences~\cite{almhiqani2020}. Variational autoencoders for synthetic data generation, domain adaptation across organizational contexts, and ensemble voting schemes have all been tried~\cite{tuor2017,gavai2015}. Still, centralized training requires data pooling, and that remains impractical in multi-organizational settings.

Federated Learning (FL) addresses this problem head-on. Under FL, each organization trains a local model on its own data and sends only parameter updates to a central server, which aggregates them into a global model and pushes it back~\cite{mcmahan2017}. Raw data never leaves the premises~\cite{kairouz2021}. Two aggregation strategies dominate the literature: FedAvg~\cite{mcmahan2017}, the original weighted-average scheme, and FedProx~\cite{li2020fedprox}, which adds a proximal term to the local objective. The proximal term penalizes local updates that stray too far from the current global model, directly counteracting the client drift that causes FedAvg to struggle when data distributions differ sharply across participants. Recent work confirms that FL-trained insider threat detectors can match centralized accuracy while complying with privacy regulations~\cite{mothukuri2021}. Beyond privacy, FL lets organizations keep full control of their infrastructure, avoid cross-border data transfer headaches, and join or leave a federation without disrupting other participants.

Despite growing FL research in network intrusion detection~\cite{popoola2021,li2020nids} and malware classification~\cite{rey2022}, its use for insider threat detection across organizationally distinct environments is still sparse. The core difficulty is that ``normal behavior'' looks fundamentally different from one industry to another. A financial analyst working at midnight to track Asian markets triggers none of the alarms that the same login pattern would raise in a manufacturing plant running fixed shifts. This kind of heterogeneity differs from the label skew studied in image classification; it stems from organizational culture, not data collection artifacts. We address this gap with the following contributions:
...
Insider threat detection has come a long way from rule-based systems. Yuan and Wu~\cite{yuan2021} used an LSTM to model temporal dependencies in user activity sequences, reaching 92.3\% accuracy on CERT. Their key insight---that recurrent architectures learn useful behavioral representations from raw log sequences without heavy feature engineering---guided our choice of LSTM as a primary candidate. Liu et al.~\cite{liu2019} took a different angle, applying an attention-enhanced CNN to behavioral feature matrices so the model could zero in on the most discriminative time windows. The attention mechanism caught pre-exfiltration reconnaissance behavior, where browsing patterns shift subtly several days before actual data theft. Le and Zincir-Heywood~\cite{le2020} combined Random Forest with autoencoder anomaly detection in an ensemble, improving recall on the malicious minority class---a stubborn problem given that malicious instances rarely exceed 5\% of records.

Chattopadhyay et al.~\cite{chattopadhyay2018} modeled user-entity relationships as graphs to detect anomalous communication patterns, with strong results on multi-insider collusion scenarios. Singh and Silakari~\cite{singh2023} surveyed 47 ML insider threat studies from 2015--2022 and found that fewer than 15\% had been validated across multiple organizations; they identified feature engineering quality and class imbalance handling as the two persistent roadblocks. Gavai et al.~\cite{gavai2015} applied isolation forests to CERT features in an unsupervised setting, showing that unsupervised methods fill a real gap when labeled malicious examples are scarce. Tuor et al.~\cite{tuor2017} paired autoencoders with recurrent networks for online detection suitable for production SOC deployment. Sanzgiri and Dasgupta~\cite{sanzgiri2016} built role-specific behavioral baselines and flagged deviations with one-class SVMs---an approach that shaped our per-client threshold calibration. Al-Mhiqani et al.~\cite{almhiqani2020} combined CNN feature extraction with LSTM temporal modeling, beating single-model approaches by 2--4\% in macro F1 on CERT. Liu et al.~\cite{liu2023} extended the line further with a dynamic framework that adapts to evolving user behavior, tackling the non-stationarity that static models handle poorly. All of these approaches, without exception, require access to raw behavioral data from one or more sources---a requirement that grows less tenable every year under tightening privacy regulations.

\subsection{Federated Learning in Cybersecurity}

FL has found applications across several cybersecurity domains. Popoola et al.~\cite{popoola2021} applied FedAvg to collaborative intrusion detection on distributed IoT networks, matching centralized accuracy while keeping network traffic data local. Their three-client setup showed that even limited collaboration improves detection of rare attack types that single clients encounter too seldom to learn from. Li et al.~\cite{li2020nids} trained federated CNNs on NSL-KDD and CICIDS, reaching centralized-level performance---but they also documented FedAvg degradation when clients had substantially different traffic patterns, a finding that directly motivated our use of FedProx.

Rey et al.~\cite{rey2022} tackled malware classification with gradient-compressed FL updates, cutting communication overhead by 80\% with minimal accuracy loss. Their work raised a practical point: in real deployments, bandwidth constraints matter as much as model quality. Mothukuri et al.~\cite{mothukuri2021} surveyed FL in cybersecurity broadly and identified data heterogeneity as the main obstacle. Their taxonomy---feature distribution skew, label skew, quantity skew, temporal skew---maps directly onto our cross-organizational scenario, which suffers from all four at once. Nguyen et al.~\cite{nguyen2019} built DIoT, a federated self-learning anomaly detector for IoT that hit 95.6\% detection with zero false positives in their evaluation. Zhao et al.~\cite{zhao2019} addressed smart grid anomaly detection under FL. Preuveneers et al.~\cite{preuveneers2018} found that local model personalization after federated training boosts detection by 3--7\% over the raw global model---a finding that fed into our threshold calibration design. Chen et al.~\cite{chen2020fedids} augmented federated IDS with GANs at each client, demonstrating that synthetic minority oversampling within a federation can partly offset class imbalance without actual data sharing.

Kim et al.~\cite{kim2022} ran a preliminary study on federated insider threat detection, but it used a single dataset, only FedAvg, and only an MLP. Cross-organizational heterogeneity and alternative aggregation strategies were left untouched. Taken together, these studies show FL's promise for cybersecurity, but insider threat detection---where behavioral baselines differ by organizational type, not just by network segment---remains underexplored.

\subsection{Federated Learning Under Non-IID Conditions}

Non-IID data across FL clients is both a theoretical and practical headache. McMahan et al.~\cite{mcmahan2017} showed that FedAvg converges well under IID conditions but can diverge badly when clients' data distributions clash, because conflicting gradient updates destabilize the global model. Li et al.~\cite{li2020fedprox} proposed FedProx to fix this: a proximal term controlled by $\mu$ that penalizes local models for drifting too far from the global model, with provable convergence under bounded heterogeneity. The penalty is especially useful when clients have small or lopsided datasets.

Karimireddy et al.~\cite{karimireddy2020} took a different route with SCAFFOLD, using control variates to estimate and correct the gap between local and global gradient directions---faster convergence than FedProx in some heterogeneity regimes. Wang et al.~\cite{wang2020} analyzed non-IID effects on FL convergence theoretically and proposed data-sharing as a mitigation for extreme cases. Li et al.~\cite{li2022fedbn} introduced Federated Batch Normalization, letting each client keep its own BN statistics while sharing all other parameters---decoupling domain-specific feature scaling from shared feature learning. Hsu et al.~\cite{hsu2019} developed a measurement framework using Dirichlet-distributed label proportions to control heterogeneity levels reproducibly. Reddi et al.~\cite{li2021fedadam} applied momentum-based server-side updates to stabilize training under heterogeneity. Zhu et al.~\cite{zhu2021} showed that ensemble knowledge distillation can outperform FedAvg under extreme non-IID conditions by transferring knowledge through soft predictions instead of raw parameter averaging.

Elshenawy et al.~\cite{elshenawy2025} provided the closest methodological precedent: a comparative FL study for multi-class breast cancer classification in ultrasound, where FedProx with MobileNet held stable across three clients with JSD up to 0.278. Their use of JSD for heterogeneity quantification and the DeLong test for AUC-ROC validation established a rigorous template that we adapt here. Our work extends this heterogeneity-aware approach into cybersecurity, where the source of non-IID conditions is qualitatively different---it arises from organizational culture and industry norms, not from imaging equipment or patient demographics.
...
where $M = \frac{1}{2}(P + Q)$ and $D_{\text{KL}}$ is the Kullback--Leibler divergence. The resulting values---0.218 (Client~1 vs.\ Client~2), 0.312 (Client~1 vs.\ Client~3), and 0.174 (Client~2 vs.\ Client~3)---confirm non-IID conditions across all pairs. These numbers sit in the same range as the heterogeneity levels Elshenawy et al.~\cite{elshenawy2025} reported for their medical imaging federation (JSD 0.158--0.278), which allows cross-domain comparison of FL behavior under comparable distributional mismatch.

Two complementary visualizations illustrate feature-level differences. Figure~\ref{fig:features} shows mean normalized values of six representative behavioral features as a grouped bar chart. The variation in Login Frequency, File Access Count, and After-Hours Ratio is immediately apparent. Figure~\ref{fig:tsne} plots a two-dimensional t-SNE embedding of the full 24-feature vectors, computed after PCA reduction to 10 components (preserving 92.4\% of variance). The three clients form tight, well-separated clusters, confirming the domain shift driven by differences in organizational operations and workforce composition.
...
Centralized training on pooled data from all three clients gives the theoretical performance ceiling. Table~\ref{tab:baseline} shows the results. LSTM leads with 76.4\% accuracy and 0.887 AUC-ROC, followed by 1D-CNN (74.8\%, 0.874) and MLP (73.1\%, 0.861). The Autoencoder, evaluated in supervised mode, trails at 71.3\% (0.842).
...
Table~\ref{tab:3c} gives the full three-client results. LSTM + FedProx tops the board at 75.8\% average accuracy and 70.4\% macro F1.
...
FedAvg's gains, solid in two-client experiments, shrink in the three-client setting---accuracy drops an average of 1.8\% relative to two-client performance. Three diverse organizational profiles produce conflicting gradient signals that FedAvg cannot reconcile. FedProx holds steady because its proximal term keeps any single client from pulling the global model too far toward its own distribution. Figure~\ref{fig:gain} shows the accuracy jump from Local to FedProx for each architecture and scenario; Figure~\ref{fig:roc} presents LSTM ROC curves across all three training paradigms.
...
How sensitive are the results to the choice of $\mu$? We ran LSTM + FedProx in the three-client federation across $\mu \in \{0.001, 0.01, 0.1, 1.0\}$. Table~\ref{tab:mu} has the numbers.
...
Table~\ref{tab:ablation} has the results.
...
The biggest winner is Client~2. Per-client thresholding lifted its accuracy from 70.1\% to 73.8\% by tuning the decision boundary to its own validation data. But local-only calibration risks overfitting on small validation sets. Federated calibration does better: by blending local thresholds with global statistics, it reaches 72.5\% average F1---4.2\% above the global baseline---without inflating false positives. Figure~\ref{fig:confusion} shows the confusion matrices: federated calibration cuts false negatives (missed malicious insiders) across all three clients.
...
We applied the DeLong test~\cite{delong1988} to compare AUC-ROC curves for the best-performing LSTM + FedProx configuration (Table~\ref{tab:delong}). The test is non-parametric and makes no assumptions about score distributions, which suits our setting well.
...
Second, FedProx significantly outperforms Local training on Clients~1 ($p = 0.008$) and~3 ($p = 0.031$). Client~2 approaches but does not cross the $\alpha = 0.05$ threshold ($p = 0.072$). This near-miss is not unusual for the smallest dataset in a federation; Elshenawy et al.~\cite{elshenawy2025} reported the same pattern with their smallest imaging dataset. Taken together with the consistent accuracy gains in Table~\ref{tab:3c}, the evidence strongly favors FedProx over isolated local training.
...
\textbf{Heterogeneity-Aware Aggregation.} FedProx won every head-to-head comparison with FedAvg, and the gap widened with heterogeneity. In the three-client setting, LSTM + FedProx hit 75.8\% average accuracy against FedAvg's 71.8\%---a 4.0-point spread attributable to the proximal term. The mechanism is straightforward: when Client~3's shift-based behavioral patterns clash with Client~1's after-hours profile, standard FedAvg lets local models over-specialize, producing updates that fight each other during aggregation. The proximal term constrains that drift. Elshenawy et al.~\cite{elshenawy2025} saw the same pattern in medical imaging with comparable JSD values, which suggests that proximal regularization is not domain-specific---it works whenever client distributions diverge meaningfully.

\textbf{Proximal Term Sensitivity.} The sweep across $\mu \in \{0.001, 0.01, 0.1, 1.0\}$ (Table~\ref{tab:mu}, Figure~\ref{fig:mu}) produced a clean inverted U. At $\mu = 0.001$, clients drift too far apart---73.2\% accuracy. At $\mu = 1.0$, the penalty crushes local adaptation, and the model cannot accommodate legitimate behavioral differences---72.4\%. The peak sits at $\mu = 0.01$ (75.8\%, F1 70.4\%), with $\mu = 0.1$ close behind (74.9\%). So the method is stable across a decade of $\mu$ values but collapses at the tails. Li et al.~\cite{li2020fedprox} predicted this: the optimal $\mu$ tracks heterogeneity, and pushing it too high penalizes useful local diversity. The worst damage at $\mu = 1.0$ fell on Client~3 (Manufacturing), whose distinctive shift patterns were most suppressed by heavy regularization.

\textbf{Client-Specific Benefits.} Federation helped clients unequally. Client~2 (Healthcare)---smallest dataset (3,847 samples), worst class balance---gained the most (+3.4\% accuracy under FedProx vs.\ Local). That matches the theoretical expectation: data-poor clients benefit most from the global model, which injects behavioral representations learned from richer datasets~\cite{kairouz2021}. Client~1 (Financial), with better local data quality, gained less. This asymmetry creates a natural incentive for participation: small organizations, facing the toughest detection problems, stand to gain the most from joining a federation.

\textbf{Centralized Training Limitations.} Centralized training did not consistently beat federated approaches, especially with three clients. The reason is negative transfer~\cite{wang2020}. Consider what happens when you pool the Financial Firm's after-hours ratio (mean 0.38---analysts tracking markets across time zones) with the Manufacturing Company's (mean 0.45---three rotating shifts). The pooled model cannot tell ``normal late work'' from ``anomalous off-shift access'' because both appear in the training set labeled as normal. A similar conflict hits file access: Healthcare's 287 files/day (EHR-driven) looks anomalous to a model calibrated on Financial's 156/day. The features confuse each other.

USB usage patterns illustrate the same problem. Client~1 averages 4.7 USB insertions daily (encrypted tokens for secure transactions). Client~3 averages 2.8 (production data transfers at shift changes). Pool them and the centralized model learns an averaged USB baseline representative of neither client. Client~1 users get flagged for ``excessive'' usage that is routine for them; Client~3's legitimate shift-boundary transfers fall below the inflated threshold. This compound feature-level confusion explains why centralized training underperformed FedProx by 3.0\% in the three-client setting despite seeing all the data. The practical takeaway: even when organizations are willing to share data, naive pooling may not help, and heterogeneity-aware FL can do better.
...
\textbf{Validity of Synthetic Benchmark Data.} The CERT dataset is synthetic. We acknowledge this openly, but it does not invalidate our results for the question we are asking. CERT has served as the primary insider threat benchmark for over a decade, appearing in more than 120 published studies~\cite{singh2023}. Its attack scenarios were designed by domain experts drawing on real cases from the intelligence community and financial sector~\cite{glasser2013,cappelli2012}. For our specific research question---can FL handle cross-organizational behavioral heterogeneity?---what matters is the distributional difference between client partitions, not whether individual log entries are synthetic or real. We engineered those differences from genuinely distinct data sources (CERT r4.2, r5.2, and LANL) with measured heterogeneity (JSD 0.174--0.312). The LANL dataset, which contains real enterprise network data, ensures that at least one client draws from authentic organizational activity. And because the community uses CERT as a common reference, our results can be compared directly against the large body of existing work.

\textbf{Threshold Calibration and Operational Relevance.} The client-adaptive threshold mechanism---our main methodological contribution---shows that tailoring decision boundaries to organizational context matters for balanced detection across heterogeneous clients. Federated calibration improved average F1 by 4.2\% over the global fixed threshold, and it preserves privacy: clients share only class-conditional means and variances, not raw predictions or model weights. In operations, a small healthcare SOC handling 50 daily alerts needs much higher precision than a large financial institution processing millions of transactions, where even a low false positive rate generates thousands of alerts. The federated calibration accommodates both within a single framework. The threshold formula (Equation~\ref{eq:threshold}) blends local and global information through $\alpha = 0.3$, selected from a sweep over $\alpha \in \{0.1, 0.3, 0.5, 0.7\}$ on validation data, where $\alpha = 0.3$ gave the best trade-off between client-specific fidelity and cross-organizational sharing.

\textbf{Comparison with Prior Work.} Our results extend FL cybersecurity findings into a harder domain. Popoola et al.~\cite{popoola2021} reached ~91\% accuracy for federated IoT intrusion detection, but with relatively homogeneous traffic. Li et al.~\cite{li2020nids} reported 93.2\% on NSL-KDD with federated CNNs, though performance dropped under non-IID conditions. Our task is fundamentally tougher: behavioral baselines differ across organizational types (not just network segments), and 4--5\% malicious rates make accuracy less informative than F1. Against Kim et al.~\cite{kim2022}, who got 68.4\% accuracy with FedAvg and a single MLP on one dataset, our LSTM + FedProx reaches 75.8\% across genuinely heterogeneous sources---7.4 points higher, due to both the better architecture and the heterogeneity-aware aggregation. FedProx's consistent superiority across our experiments reinforces the recommendation by Li et al.~\cite{li2020fedprox}: proximal regularization should be the default for heterogeneous FL.

\textbf{Scalability Considerations.} Three clients demonstrate the heterogeneity problem; real deployments would involve dozens. The Flower framework scales well: each round transmits only model parameters---about 9.4~MB for LSTM ($2 \times 128$ hidden units $\approx$ 2.35M parameters $\times$ 4~bytes/float32), 3.6~MB for MLP (256$\rightarrow$128$\rightarrow$64, $\approx$ 0.9M parameters), 6.7~MB for 1D-CNN (3 conv blocks, $\approx$ 1.68M parameters). Over 10 rounds, total transfer per client stays under 100~MB. A 10-client federation with similar architectures would remain under 1~GB total, well within enterprise network capacity. But more clients mean higher update variance, possibly requiring larger $\mu$ or alternative strategies like SCAFFOLD~\cite{karimireddy2020}. Incentive alignment also gets harder: free-riding clients that train minimally while benefiting from the global model could drag down overall quality~\cite{kairouz2021}.

\textbf{Limitations.} We note several. First, CERT is synthetic; validation on proprietary enterprise data would strengthen the conclusions, despite CERT's status as the community standard. Second, three clients is a simplified federation---real deployments involve dozens or hundreds of organizations. Third, we do not address adversarial scenarios such as model poisoning~\cite{zhang2022}. Fourth, the communication overhead of federated training was not systematically benchmarked against centralized approaches under resource constraints. Fifth, all results come from a single deterministically seeded execution (seed 42). We did not run multiple seeds because the 47 full federated experiments were computationally expensive. Standard deviations in Tables~\ref{tab:2c1}--\ref{tab:2c3} and Table~\ref{tab:3c} reflect bootstrap resampling variance (1,000 iterations on the held-out test set), not inter-run variance. Sixth, the threshold calibration mechanism was evaluated primarily at $\alpha = 0.3$, selected from a coarse sweep over $\{0.1, 0.3, 0.5, 0.7\}$; a finer or adaptive selection could yield further gains.
