...
\item A comprehensive comparison of Local, Centralized, FedAvg, and FedProx training paradigms across 47 experiments---comprising 4 centralized baselines, 9 two-client local baselines (3 pairings $\times$ 3 architectures), 18 two-client FL experiments (3 pairings $\times$ 3 architectures $\times$ 2 strategies), 3 three-client local baselines, 6 three-client FL experiments (3 architectures $\times$ 2 strategies), 3 threshold calibration variants, and 4 $\mu$ sensitivity configurations---spanning two-client and three-client federation settings, with per-client evaluation of all metrics.
\end{enumerate}
...
To visualize feature-level differences across clients, we present two complementary views. Figure~\ref{fig:features} displays the mean normalized values of six representative behavioral features across the three client partitions as a grouped bar chart. The substantial variation in feature magnitudes---particularly Login Frequency, File Access Count, and After-Hours Ratio---visually confirms the non-IID nature of the client data. To further illustrate the distributional separation, Figure~\ref{fig:tsne} presents a two-dimensional t-SNE embedding of the full 24-dimensional behavioral feature vectors, computed after PCA reduction to 10 principal components (preserving 92.4\% of variance). The resulting scatter plot reveals distinct client-specific clusters with minimal overlap, confirming the presence of domain shift arising from differences in organizational culture, operational patterns, and workforce composition.
...
\textbf{Proximal Term Sensitivity.} The dedicated sensitivity analysis across $\mu \in \{0.001, 0.01, 0.1, 1.0\}$ for LSTM in the three-client setting (Table~\ref{tab:mu}, Figure~\ref{fig:mu}) revealed a clear inverted-U relationship between $\mu$ and performance. At $\mu = 0.001$, insufficient regularization allows client drift, where local models over-specialize to their own behavioral distributions and contribute conflicting gradients during aggregation, resulting in 73.2\% average accuracy. At $\mu = 1.0$, excessive regularization suppresses beneficial local adaptation, effectively forcing all clients toward a uniform model that cannot accommodate the legitimate behavioral differences across organizational types, yielding 72.4\% average accuracy. The optimal value $\mu = 0.01$ achieved 75.8\% average accuracy and 70.4\% macro F1-score, while $\mu = 0.1$ produced competitive intermediate results (74.9\% accuracy), suggesting relative robustness within the $[0.01, 0.1]$ range. This sensitivity profile aligns with the theoretical analysis by Li et al.~\cite{li2020fedprox}, who showed that the optimal $\mu$ value depends on the degree of heterogeneity: higher heterogeneity generally benefits from stronger regularization, but excessive values prevent clients from learning local patterns that contribute useful diversity to the global model. Notably, the performance degradation at $\mu = 1.0$ was most severe for Client~3 (Manufacturing), whose distinctive shift-based patterns were most suppressed by strong regularization.
...
\textbf{Centralized Training Limitations.} Centralized training did not consistently outperform federated approaches, particularly in the three-client setting. This counterintuitive result reflects the negative transfer phenomenon, whereby naive data pooling across heterogeneous organizational profiles introduces conflicting behavioral signals~\cite{wang2020}. Consider a concrete example from our data: the Financial Firm's after-hours login ratio averages 0.38, meaning that nearly 40\% of login events occur outside standard business hours---a normal pattern for financial analysts monitoring markets across time zones. When this data is pooled with Manufacturing Company data where the after-hours ratio averages 0.45 (reflecting shift-based operations with three rotating shifts), the resulting model cannot distinguish between ``normal after-hours work'' and ``anomalous off-shift access'' because the pooled training set contains both patterns labeled as normal. A similar conflict arises with file access counts: Healthcare Organization's mean of 287 files per day (driven by EHR access) is treated as anomalous by a model calibrated on the Financial Firm's mean of 156. These conflicting signals degrade the centralized model's feature representations, leading to inferior performance compared to federated approaches that preserve client-specific behavioral baselines.

A second illustrative example involves USB device usage patterns. Client~1 (Financial) exhibits a mean of 4.7 USB insertions per day, reflecting regular use of encrypted USB tokens for secure transaction authentication. Client~3 (Manufacturing) averages 2.8 insertions per day, primarily for production data transfers at shift boundaries. When these distributions are pooled, the centralized model learns an averaged USB baseline that is neither representative of financial nor manufacturing norms, resulting in elevated false positive rates for both clients---Client~1 users are flagged for ``excessive'' USB usage that is routine in their context, while legitimate Client~3 shift-boundary transfers fall below the artificially elevated alert threshold. This feature-level confusion compounds across multiple behavioral dimensions, explaining why centralized training underperformed FedProx by 3.0\% in the three-client setting despite having access to all training data. This finding has direct practical implications: even in scenarios where organizations are willing to share data, simple pooling may not yield the expected performance improvements, and federated approaches with heterogeneity-aware aggregation can achieve superior results.

\textbf{Qualitative Case Analysis.} To provide concrete insight into model behavior, we examined individual detection outcomes in the three-client FedProx setting. The most notable success case involved a simulated financial analyst (Client~1) exhibiting a gradual increase in after-hours file access over a 12-day period, culminating in a large USB data transfer on day 13. The LSTM + FedProx model correctly flagged this user on day 10---three days before the actual exfiltration event---by detecting the behavioral drift pattern in the after-hours ratio and file access features. The local-only model, lacking exposure to complementary behavioral patterns from other organizational types, did not flag this user until day 12, one day before the exfiltration. Conversely, the most informative failure case occurred in Client~2 (Healthcare), where a clinical staff member's legitimate after-hours access to patient records during a hospital surge event was flagged as malicious by both the global model and the federated model. This false positive was generated because the behavioral pattern (sustained after-hours access combined with elevated file access counts) closely mimicked the pre-exfiltration pattern the model had learned from Client~1's financial data. The client-adaptive threshold calibration mechanism (Section~4.6) partially addressed this issue by raising Client~2's threshold to account for its naturally higher file access baseline.

\textbf{Validity of Synthetic Benchmark Data.} A legitimate concern about our experimental design is the use of the CERT dataset, which contains synthetic rather than real behavioral logs. We address this concern directly. The CERT dataset has been the primary benchmark for insider threat detection research for over a decade, used in more than 120 published studies according to a recent survey~\cite{singh2023}. The synthetic logs were generated by the CERT Division at Carnegie Mellon's Software Engineering Institute using domain expertise from intelligence community and financial sector insider threat investigations, and the malicious scenarios embedded in the dataset reflect documented attack patterns from real cases~\cite{glasser2013,cappelli2012}. Crucially, for the specific research question addressed in this paper---whether federated learning can handle cross-organizational behavioral heterogeneity---the synthetic nature of individual behavioral logs is less important than the distributional differences between client partitions, which we engineered from genuinely different data sources (CERT r4.2, r5.2, and LANL) with quantified heterogeneity (JSD 0.174--0.312). The inclusion of the LANL dataset, which contains real enterprise network data, further strengthens our generalizability claims by ensuring that at least one client draws from authentic organizational activity. Moreover, the community consensus around CERT as the standard benchmark provides an essential common reference point: our results can be directly compared against the substantial body of existing work that uses the same dataset, enabling meaningful scientific discourse about the specific contributions of federated approaches.

\textbf{Threshold Calibration and Operational Relevance.} The client-adaptive threshold calibration mechanism---our primary methodological contribution---demonstrates that adapting decision boundaries to organizational context is essential for balanced insider threat detection across heterogeneous clients. The federated calibration approach improved average F1-score by 4.2\% over the global fixed threshold while preserving the privacy properties of the framework---clients share only aggregate score distribution statistics (class-conditional means and variances) rather than raw predictions or model internals. This mechanism is particularly significant for operational deployment, where different organizations face fundamentally different base rates of insider threats. A security operations center at a small healthcare facility, handling perhaps 50 daily alerts, requires much higher alert precision than one at a large financial institution processing millions of daily transactions, where even a low false positive rate generates thousands of alerts that overwhelm analyst capacity. The federated calibration approach accommodates these different operational requirements within a unified framework, making it a practical contribution beyond the purely academic performance improvements it provides. The mathematical formulation (Equation~\ref{eq:threshold}) ensures that the calibration blends local and global information in a principled manner: the blending coefficient $\alpha = 0.3$ was selected based on a preliminary sweep across $\alpha \in \{0.1, 0.3, 0.5, 0.7\}$ on the validation sets, where $\alpha = 0.3$ achieved the best trade-off between client-specific calibration fidelity and cross-organizational information sharing.

\textbf{Comparison with Prior Work.} Our results extend the findings of prior FL studies in cybersecurity by demonstrating that the benefits of federated aggregation generalize from network intrusion detection and malware classification to the more challenging domain of insider threat detection. Popoola et al.~\cite{popoola2021} achieved approximately 91\% accuracy for federated intrusion detection on IoT network data with relatively homogeneous traffic distributions. Li et al.~\cite{li2020nids} reported 93.2\% accuracy on the NSL-KDD dataset using federated CNNs but observed degradation under non-IID conditions. Our insider threat detection task operates in a fundamentally more challenging regime: behavioral baselines differ across organizational types rather than merely across network segments, and the extreme class imbalance (4.0--5.0\% malicious rate) makes raw accuracy a less informative metric than macro F1-score. Compared to Kim et al.~\cite{kim2022}, who achieved 68.4\% accuracy using FedAvg with a single MLP on a single-dataset partition, our LSTM + FedProx framework achieves 75.8\% average accuracy across genuinely heterogeneous data sources---an improvement of 7.4 percentage points attributable to both the superior architecture and the heterogeneity-aware aggregation strategy. The consistent superiority of FedProx over FedAvg across all our experiments reinforces the recommendation by Li et al.~\cite{li2020fedprox} that proximal regularization should be the default choice for heterogeneous FL deployments.

\textbf{Scalability Considerations.} While our three-client federation demonstrates the core heterogeneity challenge, real-world deployments would involve considerably larger federations. The Flower framework we employed scales efficiently to dozens of clients with manageable communication overhead: each communication round requires only the transmission of model parameters (approximately 9.4~MB for our LSTM architecture with $2 \times 128$ hidden units $\approx$ 2.35M parameters $\times$ 4~bytes/float32, 3.6~MB for MLP with 256$\rightarrow$128$\rightarrow$64 layers $\approx$ 0.9M parameters, and 6.7~MB for 1D-CNN with 3 convolutional blocks $\approx$ 1.68M parameters), and with 10 communication rounds, the total data transfer per client amounts to less than 100~MB for the entire training process. For a 10-client federation with similar architectures, the total communication cost would remain under 1~GB, well within the capacity of standard enterprise network connections. However, increasing the number of clients introduces additional challenges: the variance of aggregated updates grows with client count, potentially requiring larger $\mu$ values or alternative aggregation strategies such as SCAFFOLD~\cite{karimireddy2020}. Furthermore, organizational incentive alignment becomes more complex in larger federations, as free-riding clients that contribute minimal local training while benefiting from the global model could degrade overall performance~\cite{kairouz2021}.

\textbf{Limitations.} Several limitations should be acknowledged. First, the CERT dataset, while the community standard for insider threat research, contains synthetic behavioral logs that may not fully capture the complexity of real organizational environments; our defense of its validity notwithstanding, validation on proprietary enterprise data would strengthen the conclusions. Second, our three-client setup, while demonstrating the heterogeneity challenge, represents a simplified federation compared to real-world deployments involving dozens or hundreds of organizations. Third, our experiments do not address adversarial attack scenarios (e.g., model poisoning~\cite{zhang2022}) that could compromise the federated training process. Fourth, the computational overhead of federated training---particularly the communication costs associated with transmitting model updates over multiple rounds---was not systematically compared against centralized approaches in resource-constrained settings. Fifth, a methodological limitation of our study is the absence of repeated experimental runs to calculate inter-run variance, as the computational resources required for full end-to-end federated experiments across all 47 scenarios were substantial. All reported results derive from a single deterministically seeded execution to ensure reproducibility, and the standard deviations reported in Tables~\ref{tab:2c1}--\ref{tab:2c3} and Table~\ref{tab:3c} reflect bootstrap resampling variance (1,000 iterations on the held-out test set) rather than inter-run variance. Sixth, the client-adaptive threshold calibration mechanism, while effective, was primarily evaluated with the blending coefficient $\alpha = 0.3$ selected from preliminary validation across $\alpha \in \{0.1, 0.3, 0.5, 0.7\}$; a more granular or adaptive $\alpha$ selection strategy could yield further improvements.

\section{Conclusions and Future Work}

This paper presented a comprehensive comparative analysis of federated learning frameworks for cross-organizational insider threat detection, addressing the critical gap between the privacy requirements of behavioral data and the data-hungry nature of modern machine learning models. Through 47 experiments across three organizationally heterogeneous clients drawn from the CERT and LANL datasets, we demonstrated that FedProx paired with LSTM achieves the best performance (75.8\% average accuracy, 70.4\% macro F1-score), outperforming local-only models by 5.4\% in accuracy and 4.9\% in macro F1-score while improving per-client macro precision by an average of 5.8\% and macro recall by 5.1\%. These values are statistically comparable to centralized training (DeLong $p > 0.05$ across all clients) while significantly outperforming local-only models ($p < 0.05$ for four of six client comparisons). The client-adaptive threshold calibration mechanism---our primary methodological contribution---further improved minority-class detection by 4.2\% in macro F1-score, providing a practical enhancement for deployment scenarios where different organizations face different base rates of insider threats while sharing only aggregate calibration statistics to preserve privacy. The proximal term sensitivity analysis confirmed that $\mu = 0.01$ is optimal for our heterogeneity regime, with performance robust within the $[0.01, 0.1]$ range.
...
Future work should explore several promising directions: (1) scaling to larger federations with 10 or more clients representing a broader range of organizational types and industry sectors, to validate the scalability of FedProx and investigate whether alternative aggregation strategies (e.g., SCAFFOLD~\cite{karimireddy2020} or FedBN~\cite{li2022fedbn}) become necessary at larger scale; (2) incorporating differential privacy mechanisms~\cite{dwork2014,geyer2017} alongside federated aggregation for formal privacy guarantees, with specific attention to the privacy-utility trade-off under the noise levels required for $(\epsilon, \delta)$-differential privacy in behavioral data; (3) investigating personalized FL approaches such as Per-FedAvg~\cite{fallah2020} and APFL~\cite{deng2020} that allow client-specific model adaptation while maintaining collaborative benefits, potentially combining personalized layers with our threshold calibration mechanism to create a fully adaptive detection pipeline; (4) extending the temporal modeling to detect slow-onset insider threats that unfold over weeks or months, which require longer observation windows and more sophisticated sequential architectures such as Transformer-based models with positional encodings adapted for irregular time series; (5) conducting a systematic exploration of the threshold calibration blending coefficient $\alpha$ and investigating adaptive $\alpha$ selection based on client-specific data characteristics; and (6) deploying the framework in a real-world enterprise pilot study with genuine organizational diversity to validate the findings under operational conditions with authentic behavioral data, including an assessment of analyst workflow integration and alert triage efficiency.
